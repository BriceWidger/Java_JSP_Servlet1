...
@WebServlet(urlPatterns = { "/DropTables" })
public class DropTables extends HttpServlet
{
    public void doGet(final HttpServletRequest request, final HttpServletResponse response) throws ServletException, IOException {
       
        //DROPS TABLE STUDENTCLASSES
        TableDrop();
        request.getRequestDispatcher("/DropTables.jsp").forward((ServletRequest)request, (ServletResponse)response);
        //DROPS TABLE CUSTOMER
        TableDrop4();
        request.getRequestDispatcher("/DropTables.jsp").forward((ServletRequest)request, (ServletResponse)response);
    }
    
    public void doPost(final HttpServletRequest request, final HttpServletResponse response) throws ServletException, IOException {
        //DROPS TABLES STUDENT
        TableDrop2();
        request.getRequestDispatcher("/DropTables.jsp").forward((ServletRequest)request, (ServletResponse)response);
        //DROPS TABLE CLASSES
        TableDrop3();
